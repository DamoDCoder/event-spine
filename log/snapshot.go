...
// Restore returns the newest snapshot and a reader positioned to replay from
// it.
//
// This is the whole documented recovery path in one call: load the state, then
// replay everything the snapshot did not fold in. Doing it by hand is two calls
// with one mistake available between them — replaying from the wrong offset —
// and the offset a snapshot carries is the first record it did *not* fold, so
// the reader starts exactly there.
//
// A log with no snapshot is not an error worth making a caller handle
// separately: it returns a zero Snapshot and a reader at the first offset the
// log still holds, which is what replaying from nothing means.
func (l *Log) Restore() (Snapshot, *Reader, error) {
	snap, err := l.LatestSnapshot()
	switch {
	case err == nil:
	case errors.Is(err, ErrNoSnapshot):
		r, err := l.Reader(l.First())
		return Snapshot{Offset: l.First()}, r, err
	default:
		return Snapshot{}, nil, err
	}

	r, err := l.Reader(snap.Offset)
	if err != nil {
		return snap, nil, err
	}
	return snap, r, nil
