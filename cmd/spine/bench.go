...
import (
	...
	"github.com/DamoDCoder/event-spine/internal/devtools/compare"
)

func benchmark(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("bench: a comparison is required, try: bench compare")
	}
	switch args[0] {
	case "compare":
		return benchCompare(args[1:])
	default:
		return fmt.Errorf("bench: unknown comparison %q", args[0])
	}
}
...
func benchCompare(args []string) error {
	...
	for _, size := range widths {
		for _, mode := range compare.Modes {
			...
			for _, backend := range []string{"spine", "kafka"} {
				...
				results := make([]compare.Result, 0, *repeat)
				var failed error
...
func runOne(backend, broker, dir string, w compare.Workload, mode compare.Mode, run int) (compare.Result, error) {
	var (
		b   compare.Backend
		err error
	)
	...
	switch backend {
	case "spine":
		b, err = compare.OpenSpine(dir, mode)
	case "kafka":
		...
		b, err = compare.OpenKafka(broker, mode, topic)
	default:
		return compare.Result{}, fmt.Errorf("unknown backend %q", backend)
	}
	if err != nil {
...
func median(results []compare.Result) compare.Result {
...
func printResult(r compare.Result) {
