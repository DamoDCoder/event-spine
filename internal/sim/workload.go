...
func Run(w Workload) (Result, error) {
	if w.Commands < 1 {
		return Result{}, fmt.Errorf("sim: commands must be at least 1, got %d", w.Commands)
	}
	if w.Accounts < 2 {
		return Result{}, fmt.Errorf("sim: accounts must be at least 2, got %d", w.Accounts)
	}

	deps, clock := Deps(w.Seed)
